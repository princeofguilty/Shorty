    ...
    class module
    {
        ...
        private static IntPtr HookCallback(int nCode, IntPtr wParam, IntPtr lParam)
        {
            ...
            else if (GetAsyncKeyState(VK_SHIFT) != 0 & GetAsyncKeyState('Q') != 0)
            {
                if (Bro.flag == true)
                {
                    Bro.logoz.BackgroundImage = Properties.Resources.logo_default;
                }
                else if (Bro.flag == false)
                {
                    Bro.logoz.BackgroundImage = Properties.Resources.logo_click_default;
                }
                Bro.flag = !Bro.flag;

                Bro.logkey.SetValue("logging", Bro.flag.ToString());

                return CallNextHookEx(_hookID, nCode, wParam, lParam);
            }
            else if (Bro.flag == true)
